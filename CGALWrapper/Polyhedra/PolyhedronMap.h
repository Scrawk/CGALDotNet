...
public:
	...
	template<class POLY>
	VERTEX* FindVertex(POLY& model, int index)
	{
		if (index == NULL_INDEX)
			return nullptr;
	...
	template<class POLY>
	FACE* FindFace(POLY& model, int index)
	{
		if (index == NULL_INDEX)
			return nullptr;
	...
	/// <param name="model">The triangulation model.</param>
	template<class POLY>
	void SetVertexIndices(POLY& model)
	{
